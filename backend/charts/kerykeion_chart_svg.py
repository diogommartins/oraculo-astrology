...
from backend.settings.kerykeion_settings import get_settings
from backend.aspects.synastry_aspects import SynastryAspects
from backend.aspects.natal_aspects import NatalAspects
from backend.astrological_subject import AstrologicalSubject
from backend.kr_types import KerykeionException, ChartType
from backend.kr_types import ChartTemplateDictionary
from backend.kr_types.settings_models import KerykeionSettingsCelestialPointModel
from backend.charts.charts_utils import (
    degreeDiff, 
    sliceToX, 
    sliceToY, 
    draw_zodiac_slice, 
...
class Chart:
    ...
    def __init__(
        ...
        self.output_directory = settings.ASTROLOGY_CHARTS_PATH
        ...
        self.template = None
        self.chartname = self.output_directory / f"{self.user.name}.{self.chart_type}.oraculo.svg"
    ...
    def image(self) -> PILImage:
        ...
        # cairosvg.svg2png(
        #     url=Path(self.chartname).as_posix(), 
        #     write_to=buffer,
        #     dpi=300,
        #     scale=2
        # )
    ...
    def _makePlanetGrid(self):
        ...
        out = '<g transform="translate(620,-10)">'
        out += '<g transform="translate(80, -15)">'
        out += f'<text text-anchor="end" style="fill:{self.chart_colors_settings["paper_0"]}; font-size: 14px;">{self.language_settings["planets_and_house"]}</text>'
        out += "</g>"
        ...
        for i in range(len(self.available_planets_setting)):
            ...
            # zodiac
            out += f'<g transform="translate(70,-8)"><use transform="scale(0.3)" xlink:href="#{self.zodiac[self.points_sign[i]]["name"]}" /></g>'
    ...
    def _draw_house_grid(self):
        ...
        """
        out = '<g transform="translate(585,190)">'
        li = 10
        for i in range(12):
            cusp = str(i + 1)
            out += f'<g transform="translate(0,{li})">'
            out += f'<text text-anchor="end" x="35" style="fill:{self.chart_colors_settings["paper_0"]}; font-size: 10px;">{self.language_settings["cusp"]}{cusp}:</text>'
    ...
    def _createTemplateDictionary(self) -> ChartTemplateDictionary:
        ...
        if self.chart_type == "Transit" or self.chart_type == "Synastry":
            ...
            td["c3style"] = f"fill: {self.chart_colors_settings['paper_1']}; fill-opacity:.8; stroke: {self.chart_colors_settings['zodiac_transit_ring_0']}; stroke-width: 2px"
            ...
            td["c3style"] = f'fill: {self.chart_colors_settings["paper_1"]}; fill-opacity:.8; stroke: {self.chart_colors_settings["zodiac_radix_ring_0"]}; stroke-width: 2px'
        ...
        if self.chart_type == "Natal" or self.chart_type == "ExternalNatal" or self.chart_type == "Synastry":
            ...
            td["bottomLeft3"] = f'{self.language_settings.get("lunar_phase", "Lunar Phase")}: {self.user.lunar_phase.moon_emoji}'
